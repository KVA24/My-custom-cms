...
const CrudModal = observer(({isOpen, type, onClose}: ModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent, type: "create" | "edit" | "delete") => {
    ...
    if (type === "delete") {
      poolStore.delete().then(() => poolStore.isOk && onClose())
      return
    }
    
    try {
      if (type === "create") {
        await storeSchema.validate(poolStore.dataRequest, {abortEarly: false})
        poolStore.errors = {}
        poolStore.create().then(() => poolStore.isOk && onClose())
      } else if (type === "edit") {
        await storeSchema.validate(poolStore.dataRequest, {abortEarly: false})
        poolStore.errors = {}
        poolStore.update().then(() => poolStore.isOk && onClose())
      }
    } catch (error: any) {
      if (error.inner) {
