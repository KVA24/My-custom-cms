"use client"

import React, {Fragment, useEffect, useState} from "react"
import {useTranslation} from "react-i18next"
import {Card, CardContent, CardHeader, CardTitle} from "@/components/ui/card.tsx"
import {Button} from "@/components/ui/button.tsx"
import {Input} from "@/components/ui/input.tsx"
import {Edit, Plus, Search, Trash2} from "lucide-react"
import CrudModal from "./components/CrudModal.tsx";
import rewardStore from "./rewardStore.ts";
import {observer} from "mobx-react-lite";
import LoadingSpinner from "@/components/common/LoadingSpinner.tsx";
import CustomPagination from "@/components/common/CustomPagination.tsx";
import {useNavigate} from "react-router-dom";
import {numberFormat} from "@/lib/utils.ts";
import {Switch, SwitchWrapper} from "@/components/ui/switch.tsx";
import BadgeRender from "@/components/common/BadgeRender.tsx";
import CustomBreadcrumb from "@/components/common/CustomBreadcrumb.tsx";

interface User {
  id: string
  name: string
  email: string
  role: string
  status: "active" | "inactive"
  avatar: string
  createdAt: string
}

const RewardsPage = observer(() => {
  const {t} = useTranslation()
  const [searchTerm, setSearchTerm] = useState("")
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [typeModal, setType] = useState<"create" | "edit" | "delete">("create")
  const navigate = useNavigate()
  
  useEffect(() => {
    rewardStore.clearState()
  }, []);
  
  function handleCreate() {
    rewardStore.clearState()
    rewardStore.getListItems().then()
    setType("create")
    setIsModalOpen(true)
    // navigate("/cms/rewards/create")
  }
  
  function handleEdit(id: string) {
    rewardStore.getListItems().then()
    rewardStore.getDetail(id).then(() => {
      setType("edit")
      setIsModalOpen(true)
    })
    // navigate(`/cms/rewards/edit/${id}`)
  }
  
  useEffect(() => {
    if (!rewardStore.searchKey) {
      rewardStore.getList().then()
      return;
    }
    
    const handler = setTimeout(() => {
      rewardStore.getList().then()
    }, 500);
    
    return () => clearTimeout(handler);
  }, [rewardStore.searchKey, rewardStore.page, rewardStore.size]);
  
  const handlePage = (page: number) => {
    rewardStore.page = page
  }
  
  const handleSize = (size: number) => {
    rewardStore.page = 0
    rewardStore.size = size
  }
  
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Rewards</h1>
          <CustomBreadcrumb
            items={[
              { label: 'Home', href: '/' },
              { label: 'Rewards', isCurrent: true },
            ]}
          />
        </div>
        <Button className="flex items-center"
                onClick={handleCreate}
                disabled={isModalOpen}>
          <Plus className="h-4 w-4"/>
          <span>{t("common.create")}</span>
        </Button>
      </div>
      
      <Card>
        <CardHeader>
          <CardTitle>Rewards List</CardTitle>
        </CardHeader>
        <CardContent>
          {/*<div*/}
          {/*  className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-2">*/}
          {/*  <div className="relative flex-1 max-w-sm">*/}
          {/*    <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400"/>*/}
          {/*    <Input
                  autoComplete="off"*/}
          {/*      placeholder={t("common.search")}*/}
          {/*      value={rewardStore.searchKey}*/}
          {/*      onChange={(e) => rewardStore.searchKey = e.target.value}*/}
          {/*      className="pl-10"*/}
          {/*    />*/}
          {/*  </div>*/}
          {/*  <div className="flex gap-2">*/}
          {/*  */}
          {/*  </div>*/}
          {/*</div>*/}
          {rewardStore.isLoading ?
            <div className="w-full flex justify-center">
              <LoadingSpinner size={"md"}/>
            </div>
            :
            <Fragment>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                  <tr>
                    <th>ID</th>
                    <th>Reward Name</th>
                    <th>Reward Type</th>
                    <th>Item Name</th>
                    <th>Value</th>
                    <th>Value Converted</th>
                    <th>Is Default</th>
                    <th>Action</th>
                  </tr>
                  </thead>
                  <tbody>
                  {(rewardStore.lists || []).map((item, index) => (
                    <tr
                      key={index}
                      className=""
                    >
                      <td>
                        {item.id}
                      </td>
                      <td>
                        {item.rewardName}
                      </td>
                      <td>
                        {item.type}
                      </td>
                      <td>
                        {item.itemName}
                      </td>
                      <td>
                        {numberFormat(item.value)}
                      </td>
                      <td>
                        {numberFormat(item.valueConverted)}
                      </td>
                      <td>
                        <BadgeRender status={item.isDefault}/>
                      </td>
                      <td>
                        <div className="flex items-center gap-2">
                          <Button
                            variant="primary"
                            size="sm"
                            data-tooltip-id="app-tooltip"
                            data-tooltip-content={t("common.edit")}
                            loading={rewardStore.isLoadingGet}
                            onClick={() => handleEdit(item.id)}
                          >
                            <Edit className="h-4 w-4"/>
                          </Button>
                          <Button
                            variant="danger"
                            size="sm"
                            data-tooltip-id="app-tooltip"
                            data-tooltip-content={t("common.delete")}
                            onClick={() => {
                              rewardStore.id = item.id
                              setType("delete")
                              setIsModalOpen(true)
                            }}
                          >
                            <Trash2 className="h-4 w-4"/>
                          </Button>
                        </div>
                      </td>
                    </tr>
                  ))}
                  </tbody>
                </table>
              </div>
              
              {rewardStore.lists.length === 0 && (
                <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                  {searchTerm ? "No results found" : "No data available"}
                </div>
              )}
            </Fragment>
          }
          <CustomPagination
            currentPage={rewardStore.page}
            totalPages={rewardStore.totalPages}
            onChangePage={handlePage}
            currentSize={rewardStore.size}
            onChangeSize={handleSize}
          />
        </CardContent>
      </Card>
      
      <CrudModal isOpen={isModalOpen} type={typeModal} onClose={() => setIsModalOpen(false)}/>
    </div>
  )
})

export default RewardsPage
