...
const CrudModal = observer(({isOpen, type, onClose}: ModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent, type: "create" | "edit" | "delete") => {
    ...
    if (type === "delete") {
      eventStore.delete().then(() => eventStore.isOk && onClose())
      return
    }
    
    try {
      if (type === "create") {
        await storeSchema.validate(eventStore.dataRequest, {abortEarly: false})
        eventStore.errors = {}
        eventStore.create().then(() => eventStore.isOk && onClose())
      } else if (type === "edit") {
        await storeSchema.validate(eventStore.dataRequest, {abortEarly: false})
        eventStore.errors = {}
        eventStore.update().then(() => eventStore.isOk && onClose())
      }
    } catch (error: any) {
      if (error.inner) {
